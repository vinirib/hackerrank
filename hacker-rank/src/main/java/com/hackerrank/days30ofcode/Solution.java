...
/**
 * 
 * Objective Today, we're discussing data types. Check out the Tutorial tab for
 * learning materials and an instructional video!
 * 
 * Task Complete the code in the editor below. The variables , , and are already
 * declared and initialized for you. You must:
 * 
 * Declare variables: one of type int, one of type double, and one of type
 * String. Read lines of input from stdin (according to the sequence given in
 * the Input Format section below) and initialize your variables. Use the
 ...
 * Input Format
 * 
 * The first line contains an integer that you must sum with . The second line
 * contains a double that you must sum with . The third line contains a string
 * that you must concatenate with .
 * 
 * Output Format
 * 
 ...
 * Explanation
 * 
 * When we sum the integers and , we get the integer . When we sum the
 * floating-point numbers and , we get . When we concatenate HackerRank with is
 * the best place to learn and practice coding!, we get HackerRank is the best
